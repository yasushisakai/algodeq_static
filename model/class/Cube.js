function Cube(_index,_room_type) {

    if(typeof _index === 'undefined') this.index = [0,0,0];
    else this.index = _index;

    if(typeof _room_type === 'undefined') this.room_type = 0;
    else this.room_type = _room_type;
...
Cube.prototype.create = function () {
    ...
    this.mesh = new THREE.Mesh(Cube.geometry, Cube.material_types[this.room_type]);
    scene.add(this.mesh);

    // updating plan occurs in update_pos, since the position is not resolved yet.
...
Cube.prototype.update_pos = function(){
    ...
    plan.add_room(this.get_index(),this.room_type);
